...
@if(Auth::check() && Auth::user()->email === config('admin.email'))
    ...
    .modal-content button:hover {
      background-color:rgb(139, 239, 141);
    }
...
@if(Auth::check() && Auth::user()->email === config('admin.email'))
  ...
  <div id="courseModal" class="modal">
    <div class="modal-content">
      ...
      <form id="addCourseForm" action="{{ route('course2.store') }}" method="POST" enctype="multipart/form-data">
  @csrf
...
@if(Auth::check() && Auth::user()->email === config('admin.email'))
         <img style="background-color:red;" 
            ...
            onclick="confirmDelete('{{ route('course2.destroy', $course->id) }}')" alt="">

        <img style="background-color:rgb(139, 239, 141);" 
    src="https://i.postimg.cc/jdqtTChF/images-removebg-preview-2.png"
    onclick="confirmEdit('{{ route('course2.update', $course->id) }}', '{{ $course->id }}', '{{ addslashes($course->name) }}', '{{ asset('storage/' . $course->pdf) }}')"
    alt="edit">
    @endif
@endauth
</div>
...
<a href="{{ route('course2.download', $course->id) }}" class="view-btn" >Télécharger</a>
  </div>
@endforeach
