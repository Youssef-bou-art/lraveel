  ...
  @endauth
   ...
   <div id="courseModal" class="modal">
    <div class="modal-content">
     ...
     <form id="addCourseForm" action="{{ route('efm2.store') }}" method="POST" enctype="multipart/form-data">
         @csrf
...
@if(Auth::check() && Auth::user()->email === config('admin.email'))
        <img style="background-color:red;" 
            ...
            onclick="confirmDelete('{{ route('efm2.destroy', $item->id) }}')" alt="">

        <img style="background-color:rgb(139, 239, 141);" 
    src="https://i.postimg.cc/jdqtTChF/images-removebg-preview-2.png"
    onclick="confirmEdit('{{ route('efm2.update', $item->id) }}', '{{ $item->id }}', '{{ addslashes($item->name) }}', '{{ asset('storage/' . $item->pdf) }}')"
    alt="edit">
...
@endauth
    ...
    <a href="{{ route('efm2.download', $item->id) }}" class="download-btn">Télécharger</a>
  </div>
@endforeach
  </div>
  </div>
