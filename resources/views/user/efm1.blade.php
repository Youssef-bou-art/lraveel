...
<head>
  ...
  <title>Digi_Learn - EFMs1</title>
  <style>
    /* Même style que la page des cours */
    * {
...
<a href="javascript:void(0);" onclick="openModal('profileModal')">Profil</a>
 ...
 @endif
  ...
  <h1>Examens de Fin de Module (EFM)</h1>

  <input type="text" id="searchInput" placeholder="Rechercher un EFM..." onkeyup="filterCourses()"/>

  @auth
@if(Auth::check() && Auth::user()->email === config('admin.email'))
 <a href="javascript:void(0);" class="add-course-btn" onclick="openCourseModal()">Ajouter EFM</a>
    @endif
  @endauth

<div id="courseModal" class="modal">
    <div class="modal-content">
        <h2>Ajouter un Nouveau EFM</h2>
        <form id="addCourseForm" action="{{ route('efm1.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <input type="text" name="name" placeholder="Nom du EFM" required />
...
@if(Auth::check() && Auth::user()->email === config('admin.email'))
         <img style="background-color:red;" 
            ...
            onclick="confirmDelete('{{ route('efm1.destroy', $item->id) }}')" alt="">

        <img style="background-color:rgb(139, 239, 141);" 
    src="https://i.postimg.cc/jdqtTChF/images-removebg-preview-2.png"
    onclick="confirmEdit('{{ route('efm1.update', $item->id) }}', '{{ $item->id }}', '{{ addslashes($item->name) }}', '{{ asset('storage/' . $item->pdf) }}')"
    alt="edit">

    @endif
@endauth
</div>
     <h2>EFM {{ $loop->iteration }}</h2>
    <p>{{ $item->name }}</p>
    <a href="{{ asset('storage/' . $item->pdf) }}" class="view-btn" target="_blank">Voir</a>
    <a href="{{ route('efm1.download', $item->id) }}" class="download-btn">Télécharger</a>
  </div>
@endforeach
  </div>
  ...
  <div class="modal-content">
    <h2>Êtes-vous sûr de vouloir supprimer ce EFM ?</h2>
...
<div id="editModal" class="modal">
  <div class="modal-content" id="editModalContent">
    <!-- أول مرة كيبان تأكيد -->
    <h2>Voulez-vous modifier ce EFM ?</h2>
